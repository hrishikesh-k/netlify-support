<script setup lang="ts">
  ...
  const pvButtonAvatarProps : ButtonProps = {
    ...
    'aria-haspopup': true,
    pt: {
      root: 'bg-transparent border-0 border-rounded-full cursor-pointer p-0 ring-offset-1 ring-offset-neutral-light-100 dark:ring-offset-neutral-dark-800 ring-teal-300 dark:ring-teal-100 hover:ring-2 transition-duration-250'
    }
  }
  const pvButtonLogoutProps : ButtonProps = {
    label: 'Logout',
    pt: {
      root: 'bg-transparent dark:hover:bg-red-900 hover:bg-red-100 border-0 border-rounded-b-2 box-border cursor-pointer flex font-inherit font-500 gap-x-2 items-center p-x-4 p-y-2 text-inherit dark:hover:text-red-200 hover:text-red-800 text-4 text-left transition-duration-250 w-full'
    }
  }
  const pvMenuProps : MenuProps = {
    ...
    pt: {
      ...
      menuitem: 'dark:hover:bg-neutral-dark-600 hover:bg-neutral-light-200 transition-duration-250 w-full',
      root:  {
        class: 'bg-neutral-000 dark:bg-neutral-dark-700 border-1 border-neutral-light-300 border-rounded-2 border-solid dark:border-neutral-dark-600',
        style: {
          'margin-top': '0.125rem'
        }
      },
      separator: 'border-0 border-neutral-light-300 border-solid border-t-1 dark:border-neutral-dark-400 m-x-2',
  ...
  const systemStatusIndicator = computed<SystemStatusIndicator>(() => {
    ...
    if (systemStatus.value === 'error') {
      ...
      iconSize = 3
      label = 'Retry'
      pt.root += 'bg-purple-100 dark:bg-purple-900 dark:hover:bg-purple-800 hover:bg-purple-200 border-purple-400 dark:border-purple-500 text-purple-800 dark:text-purple-200'
      ...
      switch (systemStatus.value.status.indicator) {
        case 'critical':
        case 'major':
          pt.root += 'bg-red-100 dark:bg-red-900 dark:hover:bg-red-800 hover:bg-red-200 border-red-400 dark:border-red-500 text-red-800 dark:text-red-200'
          severity = 'error'
          break
        case 'maintenance':
          pt.root += 'bg-blue-100 dark:bg-blue-900 dark:hover:bg-blue-800 hover:bg-blue-200 border-blue-400 dark:border-blue-500 text-blue-800 dark:text-blue-200'
          severity = 'info'
          break
        case 'minor':
          pt.root += 'bg-gold-100 dark:bg-gold-900 dark:hover:bg-gold-800 hover:bg-gold-200 border-gold-400 dark:border-gold-500 text-gold-800 dark:text-gold-200'
          severity = 'warn'
          break
        case 'none':
          pt.root += 'bg-green-100 dark:bg-green-900 dark:hover:bg-green-800 hover:bg-green-200 border-green-400 dark:border-green-500 text-green-800 dark:text-green-200'
          severity = 'success'
          break
      }
    } else {
      pt.root += 'bg-neutral-light-200 dark:bg-neutral-dark-700 border-neutral-light-400 dark:border-neutral-dark-500 cursor-wait text-neutral-light-800 dark:text-neutral-dark-200'
    }
    return {
      component: {
        name: componentName,
